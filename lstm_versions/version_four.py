...
logger = logging.getLogger('version_four.py')
...
class LstmV4(object):
    ...
    def fit_model(self):
        ...
        with open(f'hyper_parameter_tuning/model_4/tuner_model_four_' + str(current_time.strftime('%d-%m-%Y_%H-%M-%S')) + '.pickle', 'wb') as file:
    ...
    def print_summary(self):
        ...
        plt.savefig('hyper_parameter_tuning/model_4/plots/lstm_model_version_four_accuracy_' + str(current_time.strftime('%Y-%m-%d_%H-%M-%S')) + '.png', bbox_inches='tight')
        plt.show()
        ...
        plt.xlabel('Epoch')
        plt.legend(['Train', 'Test'], loc='upper left')
        plt.savefig('hyper_parameter_tuning/model_4/plots/lstm_model_version_four_loss_' + str(current_time.strftime('%Y-%m-%d_%H-%M-%S')) + '.png', bbox_inches='tight')
        plt.plot()
    ...
    def save_model(self):
        ...
        model_name = f'hyper_parameter_tuning/model_4/best_model/lstm_version_four_best_model_' + str(current_time.strftime('%Y-%m-%d_%H-%M-%S')) + '.json'
        weight_name = f'hyper_parameter_tuning/model_4/best_model/lstm_version_four_best_model_' + str(current_time.strftime('%Y-%m-%d_%H-%M-%S')) + '.h5'
        with open(model_name, "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        self.lstm_model.save_weights(weight_name)
        print("Saved lstm_version_four_best_model to disk")

    def load_model(self):
        loaded_tuner = pickle.load(open("D:/Thesis/recsys_thesis/lstm_versions/hyper_parameter_tuning/model_4/tuner_model_four_02-05-2020_19-26-02.pickle", "rb"))
...
lstm_version_4 = LstmV4(sequence_limit=2, test_size=0.20)
lstm_version_4.create_sequences()
lstm_version_4.prepare_data()

if not train_model:
    lstm_version_4.load_model()
else:
    lstm_version_4.fit_model()
